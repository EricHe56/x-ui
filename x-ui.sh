...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/EricHe56/x-ui/new-x-ui/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://raw.githubusercontent.com/EricHe56/x-ui/new-x-ui/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
